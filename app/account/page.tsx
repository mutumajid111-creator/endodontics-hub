...
type PaymentRow={id:string;status:string;amount:number|null;currency:string;created_at:string;payer_reference:string|null;subscription_plans:{name:string}|null};
...
export default function AccountPage(){
 ...
 useEffect(()=>{async function load(){const {data:{user}}=await supabase.auth.getUser(); if(!user){router.replace("/member/login?next=/account");return;} setEmail(user.email||""); const [{data:s},{data:p}]=await Promise.all([supabase.from("subscriptions").select("status,starts_at,ends_at,subscription_plans(name,interval)").eq("user_id",user.id).order("created_at",{ascending:false}).limit(1).maybeSingle(),supabase.from("payment_transactions").select("id,status,amount,currency,created_at,payer_reference,subscription_plans(name)").eq("user_id",user.id).order("created_at",{ascending:false}).limit(8)]); setSub((s as SubRow|null)||null); setPayments((p||[]) as PaymentRow[]); setLoading(false);} void load();},[router]);
