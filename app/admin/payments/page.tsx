...
export default function AdminPaymentsPage(){
 const router=useRouter(); const [items,setItems]=useState<Payment[]>([]); const [users,setUsers]=useState<UserMap>({}); const [message,setMessage]=useState(""); const [loading,setLoading]=useState(true);
 ...
 return <main className="adminPremiumPage">
  ...
  <section className="shell adminPaymentsList">
   ...
   {loading?<p>Loading payments...</p>:items.length===0?<div className="emptyPremium"><h2>No payment requests yet.</h2><p>New Qi submissions will appear here automatically.</p></div>:items.map(item=><article className="paymentAdminCard" key={item.id}>
    <div className="paymentAdminMain"><div className="paymentStatusRow"><span className={`premiumStatus ${item.status}`}>{item.status}</span><small>{new Date(item.created_at).toLocaleString()}</small></div><h2>{item.subscription_plans?.name||"Membership"}</h2><div className="paymentFacts"><span><small>AMOUNT</small><b>{item.amount==null?"—":`${Number(item.amount).toLocaleString()} ${item.currency}`}</b></span><span><small>REFERENCE</small><b>{item.payer_reference||"Not entered"}</b></span></div>{item.review_note&&<p className="reviewNote">{item.review_note}</p>}</div>
